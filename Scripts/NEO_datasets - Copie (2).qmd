...
bo_field<- map(bo_set,~field_sel(.,bo_setvar))
```
...
po_field<- map(po_set,~field_sel(.,po_setvar))
```
...
sl_field<- map(sl_set,~field_sel(.,sl_setvar))
```
...
cf_field<- map(cf_set,~field_sel(.,cf_setvar))
```
...
SNO_Areas_S <- SNO_Areas_S_tmp %>%
  ...
  rename(Int_Area_Small=Int_Area) %>% 
...
bo_set <- bo_set %>% map(.,left_join,sno_coord, 
                         by = c("bdd_base", "station", "latitude", "longitude"))
...
areas_L<-SNO_Areas_S %>%
  st_drop_geometry(SNO_Areas_S) %>% 
  distinct(Int_Area_Large) %>% 
  pull %>% 
  as.list
areas_S<-SNO_Areas_S %>%
  st_drop_geometry(SNO_Areas_S) %>% 
  arrange(id) %>% 
  select(Int_Area_Large,Int_Area_Small) %>% 
  split(.$Int_Area_Large) %>%
  modify_depth(1,as.list) %>% 
  modify_depth(1,~discard_at(.,"Int_Area_Large")) %>% 
  list_flatten(.,name_spec = "{outer}") %>%
  modify_depth(1,as.list)
areas_S<-areas_S[SNO_Areas_S %>%
                           distinct(Int_Area_Large) %>%
                           pull]
...
sno_S_filt<-function(snoset,IntAreaSmall){
  map(snoset, ~ filter(.,Int_Area_Small==IntAreaSmall)) %>% 
  keep(~dim(.)[1]>0)
}
...
sno_set_S<-map(areas_S,~sno_set_areas(sno_set,.x))
...
time_S<-function(timeline,IntAreaLarge) {  # IntAreaLarge="Manche"
  gp<- timeline %>% 
      filter(Int_Area_Large %in% IntAreaLarge) %>% 
...
::: panel-tabset
```{r}
#| label: fig-S_focus120
#| include: true
#| fig-width: 16
#| fig-height: 10
#| results: asis

iwalk(focus_S_overview, ~ {
  cat('### ', .y, '\n\n')
  print(.x$t)
  print(.x$m)
  cat('\n\n')
...
focus_M_overview[[1]]$t
focus_M_overview[[1]]$m
```
...
focus_M_overview[[2]]$t
focus_M_overview[[2]]$m
```
...
focus_M_overview[[3]]$t
focus_M_overview[[3]]$m
```
...
focus_M_overview[[4]]$t
focus_M_overview[[4]]$m
```
:::
...
time_S_z<-function(IntAreaSmall,timeline) { 
  gp<-timeline %>% 
    filter(Int_Area_Small==IntAreaSmall) %>%
    ggplot(aes(x=station, y=sampling_date, color=bdd_base)) +
    coord_flip() +
    scale_colour_manual(values=SNO_pal) +
    geom_jitter(size = 2, alpha = 0.4, width = 0.2) +
...
overview_S_z<-function(IntAreaLarge,timeline,sno_map,SNO_Areas){
  seareas_L<- areas_sum %>%
    ...
    filter(Int_Area_Large==IntAreaLarge) %>%
    drop_na(Int_Area_Small) %>% 
    distinct(Int_Area_Small) %>%
    arrange(Int_Area_Small) %>% 
    pull %>% 
    as.list
...
zoom_S_overview<-map(areas_L,~overview_S_z(.,timeline,sno_map,SNO_Areas)) 
names(zoom_S_overview)<-unlist(areas_L)
...
zoom_S_overview[[1]][[3]]$t
zoom_S_overview[[1]][[3]]$m
```

::: column-page
::: panel-tabset
```{r}
#| label: fig-S_focus130
#| include: true
#| fig-width: 16
#| fig-height: 10
#| results: asis
