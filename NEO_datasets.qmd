...
tm_data<-
  ...
  tm_shape(sno_coord) +
    tm_dots(jitter = 0.01,col="bdd_base", palette=colbdd,size=0.1,clustering = F) +
...
for (dfi in 1:length(df_set)) {
  ...
  df_all_desc[[dfi]]<-list(bdd_base =bdd_base,
                       df_name=df_name,
                       obsnbtot=obsnbtot,
                       varnbtot=varnbtot,
...
df_all_desc <- bind_rows(df_all_desc) 
df_all_desc %>% kable()

timevision <- df_set %>% lapply(. %>% 
  ...
  coord_flip() +
  Scalc_lon(4) +
  facet_wrap(~bdd_base, scale="free") +
